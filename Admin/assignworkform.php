...
if(isset($_SESSION['is_adminlogin'])){
 ...
 if(isset($_REQUEST['assign'])){
  if(($_REQUEST['request_id'] == "") || ($_REQUEST['request_info'] == "") || ($_REQUEST['requestdesc'] == "") || ($_REQUEST['requestername'] == "") || ($_REQUEST['address1'] == "") || ($_REQUEST['address2'] == "") || ($_REQUEST['requestercity'] == "") || ($_REQUEST['requesterstate'] == "") || ($_REQUEST['requesterzip'] == "") || ($_REQUEST['requesteremail'] == "") || ($_REQUEST['requestermobile'] == "") || ($_REQUEST['assigntech'] == "") || ($_REQUEST['inputdate'] == "")){
   ...
   $time = $_REQUEST['time'];
   $sql = "INSERT INTO assignwork_tb (request_id, request_info, request_desc, requester_name, requester_add1, requester_add2, requester_city, requester_state, requester_zip, requester_email, requester_mobile, assign_tech, assign_date, status, time) VALUES ('$rid', '$rinfo','$rdesc', '$rname', '$radd1', '$radd2', '$rcity', '$rstate', '$rzip', '$remail', '$rmobile', '$rassigntech', '$rdate','$status','$time')";
   if($conn->query($sql) == TRUE){
 ...
 <form action="" method="POST">
   ...
   <div class="form-row">
    <div class="form-group col-md-6">
      ...
      <select class="form-control m-0 p-0 " name="assigntech" id="assigntech">
        ...
        ?>
            <option value="<?php echo $empid["empName"];?>"><?php echo $empid["empName"];?></option>
            <?php
                }
            ?>
      </select>
    ...
    <div class="form-group col-md-6">
      ...
      <input type="text" class="form-control" value="<?php echo $row['request_date'];  ?>" id="inputDate" name="inputdate">
    </div>
	 <div class="form-group col-md-6">
      <label for="inputDate">Time</label>
      <input type="time" class="form-control" id="time" value="<?php echo $row['time'];  ?>"name="time">
    </div>
	 <div class="form-group col-md-6">
      <label for="inputDate">Payment Status</label>
      <input type="text" class="form-control" id="time" value="<?php 
	  include('../dbConnection.php');
	  $r=$row['request_id'];
	  $sql = "SELECT * FROM assignwork_tb where request_id='$r'";
$result = $conn->query($sql);
...
if ($result->num_rows > 0) {
  ...
  while($rowss = $result->fetch_assoc()) {
   ...
   <div class="float-right">
    <button type="submit" class="btn btn-success" name="assign">Submit</button>
